...
SEXP SIMDAYSMOOTHLEVELINPUT2(SEXP Y, SEXP L, SEXP DAYS, SEXP S, SEXP OPTNOUT, SEXP PARAM, SEXP THOLD, SEXP STARTVAL) {
	...
	for(int i=1;i<(n+f);i++){
		...
		if(i<n){
			...
			// w1 and w2 seemed to converge to 0.5 and 0.5
...
SEXP SIMDAYSMOOTH3(SEXP Y, SEXP DAYS, SEXP S, SEXP OPTNOUT, SEXP PARAM, SEXP THOLD, SEXP STARTVAL) {
	...
	for(int i=1;i<(n+f);i++){
		...
		if(i<n){
			...
			if(i<=(n-o)){ //Make predictions "o" steps ahead
				for(int j=0; j<o; j++){
					d=nvDAYS(i+j);
					nvFIL(i, j)=dL+(j+1)*dT+nvS(d); //Predicted/Filtered value for "today"
				}
				d = nvDAYS(i);
			}else{
			...
			dT=beta*(dL-dL1)+(1-beta)*dT;
			nvS(d)=gamma*(xhat-dL)+(1-gamma)*nvS(d); 	//Trend updated with change in level
			dL1=dL;
			
		}else{
			nvFIL(i, 0) = dL+(i-n+1)*dT+nvS(d);
		}
	}
...
// ----------------- This model calculates OPTNOUT predictions at each step in filtration -----------------------------
// ------------------------ allowing for more flexible optimization routines ------------------------------------------
SEXP SIMDAYSMOOTHLEVELINPUT3(SEXP Y, SEXP L, SEXP DAYS, SEXP S, SEXP OPTNOUT, SEXP PARAM, SEXP THOLD, SEXP STARTVAL) {
	...
	double dT = nvSTARTVAL(2); //Trend
	double dL1 = dL; // smoothed level
	double dL1_1 = dL; // previous smoothed level
	NumericVector nvL(L);
	double dL2 = 0; // input level
	double r1=0.5; // accuracy of smoothed level
	double r2=0.5; // accuracy of input level
	...
	for(int i=1;i<(n+f);i++){
		...
		if(i<n){
			...
			dL1=alfa*(xhat-nvS(d))+(1-alfa)*(dL1+dT); 	//Smoothed level updated with value of today
			dL2=nvL(i);
			dL=w1*dL1+w2*dL2;
			
			dT=beta*(dL1-dL1_1)+(1-beta)*dT;		//Update Trend
			dL1_1=dL1;
