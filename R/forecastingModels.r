...
OPTsimdaysmooth3 <- function(y, ymat, days, s, opt.nout, thold, param, startVal, scorefunc, trim){
	n <- length(y)

	fitval <- .Call("SIMDAYSMOOTH3", Y=y, DAYS=days, S=s, OPTNOUT=opt.nout, PARAM=param,
			THOLD=thold, STARTVAL=startVal, PACKAGE = "predX")
...
simdaysmooth3 <- function(y, days, param=NULL, doOptim=TRUE, thold=2, opt.nout=7,
			scorefunc=fMSE, trim=0, solver.method="Nelder-Mead", solver.control=list()){
			
	n <- length(y); nout <- length(days)-n; s <- length(unique(days))
	
	startVal = rep(NA, s+3) #Level0 and Seas1:(s+1)
	startVal[1] <- var(y); startVal[2] <- mean(y[1:10]); startVal[3] <- 0;
	nn <- min(10*s, n) #Number of days used of initializing seasonal component
	startVal[4:(s+3)] <- aggregate(y[1:nn], by=list(days[1:nn]), mean)$x-mean(y[1:nn])
	
	if(is.null(param)){param <-  INVunityf(c(0.5, 0.5, 0.1))
	}else{param <- INVunityf(param)}
	...
	if(doOptim){
		...
		opt <- optim(param, OPTsimdaysmooth3, y=y, ymat=ymat, days=days[1:n], s=s, opt.nout=opt.nout, 
				startVal=startVal, scorefunc=scorefunc, trim=trim, thold=thold, 
				method=solver.method, control=solver.control)
	...
	fit <- .Call("SIMDAYSMOOTH3", Y=y, DAYS=days, S=s, OPTNOUT=1, PARAM=param, 
			THOLD=thold, STARTVAL=startVal, PACKAGE = "predX" )
...
OPTsimdaysmoothlevelinput3 <- function(y, ymat, l, days, s, opt.nout, thold, param, startVal, scorefunc){
	n <- length(y)

	fitval <- .Call("SIMDAYSMOOTHLEVELINPUT3", Y=y, L=l, DAYS=days, S=s, OPTNOUT=opt.nout, PARAM=param,
			THOLD=thold, STARTVAL=startVal, PACKAGE = "predX")
...
simdaysmoothlevelinput3 <- function(y, l, days, param=NULL, doOptim=TRUE, thold=2, opt.nout=7,
				scorefunc=fMSE, solver.method="Nelder-Mead", solver.control=list()){
	...
	if(doOptim){
		...
		opt <- optim(param, OPTsimdaysmoothlevelinput3, y=y, ymat=ymat, l=l, days=days[1:n], s=s, 
				opt.nout=opt.nout, 
				startVal=startVal, scorefunc=scorefunc, thold=thold, 
				method=solver.method, control=solver.control)
	...
	fit <- .Call("SIMDAYSMOOTHLEVELINPUT3", Y=y, L=l, DAYS=days, S=s, OPTNOUT=1, PARAM=param, 
			THOLD=thold, STARTVAL=startVal, PACKAGE = "predX" )
